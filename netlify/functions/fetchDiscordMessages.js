...
async function fetchMessagesFromDiscord(threadId, userName, lastMessageId, repUserId) {
    ...
    try {
        ...
        .filter(msg => (msg.content.trim() !== '' && msg.content !== '[INITIAL_MESSAGE]') || (msg.attachments && msg.attachments.length > 0) || (msg.embeds && msg.embeds.length > 0))
        .map(msg => ({
            id: msg.id,
            ...
            embed: msg.embeds && msg.embeds.length > 0 ? msg.embeds[0] : null
        }))
        .reverse();
